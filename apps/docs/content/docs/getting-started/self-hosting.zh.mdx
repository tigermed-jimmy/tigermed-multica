...
<Callout>
If the self-host server is already running and you only need the CLI on a macOS/Linux machine, install it with Homebrew: `brew install tigermed-jimmy/tap/multica`.
</Callout>
