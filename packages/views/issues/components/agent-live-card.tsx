          ...
          <>
            ...
            {expanded && (
              <div className="divide-y divide-info/15 border-t border-info/15">
                {entries.map(({ task, messages, activity }) => (
                  <AgentLiveRow
                    key={task.id}
                    task={task}
...
function AgentLiveRow({ task, items, messages, activity, agentName, onRequestCancel, cancelling }: AgentLiveRowProps) {
  const { t } = useT("issues");
    ...
    <div className="flex items-center gap-2 px-3 py-2 text-muted-foreground">
      ...
      <div className="flex items-center gap-1.5 text-xs min-w-0">
        ...
        {/* Running tasks: surface the live stage (running command / reading
            ...
            the banner doesn't look frozen between tool calls. The row's own
